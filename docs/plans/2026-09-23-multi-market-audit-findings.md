...
| H6 | Ví chặn method tùy chỉnh ở client (Ambire, một số build MetaMask/Rabby): `method [morpho_proxyInfo] doesn't has corresponding handler` ⇒ user tưởng proxy sai dù RPC chưa hề được gọi | ⚠️ Không thể sửa client-side — đã gỡ gate (2026-09-24); **Ambire đã xác thực ký thật thành công cùng ngày** | Bằng chứng thực tế: Ambire v6.21.4 tự trả lời `web3_clientVersion` ("Ambire v6.21.4") ⇒ không có probe client-side nào xác thực được RPC qua ví; flow ký khôi phục như main; **capture thành công (from = LENDER_ADDRESS)** ⇒ Ambire ký qua đường raw/EOA, tương thích kiến trúc presign; nút Thêm Mạng Proxy vẫn giữ hướng dẫn thủ công khi bị chặn |
| H7 | Single-nonce limitation: registry khóa bundle theo `marketId` + broadcast chỉ nhận bundle `nonce == pending` ⇒ phải chờ bundle trước mine mới ký được market kế tiếp | ✅ Đã sửa (2026-09-24) — registry v3 multi-nonce ladder | Key mới `marketId@nonce` (identity đọc từ VALUE: `bundle.marketId`+`bundle.nonce`, key là opaque); v2 auto-migrate in-memory (giữ key, stamp marketId); broadcast giữ nguyên mọi invariant — chỉ claim `nonce==on-chain pending` ⇒ tự động nonce tăng dần; hết hạn theo on-chain nonce (pending ≤ consumed → expired); cùng nonce = race, market trigger trước thắng (markets.json giải hòa cùng chu kỳ), mine xong sibling expired; KHÔNG pre-broadcast future nonce (tx future-nonce mine vô điều kiện, phá điều kiện trigger); webapp: GET ladder + overview rounds + confirm khi ký trùng nonce; 5 test mới (`__tests__/presigned-lifecycle.test.mjs`) |
