...
# Static gates
npm run lint         # oxlint --deny no-undef --deny-warnings .  (bắt lớp lỗi C1: identifier đã bị xoá)
npm run check        # lint + node --check (.mjs + script nội tuyến của webapp.html) + vitest run

# Tests
npm test             # vitest run (all 527 tests across 35 files; 520 chạy + 7 skipped = live ntfy, NTFY_LIVE=1 để chạy)
npx vitest run       # same
