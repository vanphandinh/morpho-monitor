...
# Tests
npm test             # vitest run (all 516 tests across 32 files; 509 chạy + 7 skipped = live ntfy, NTFY_LIVE=1 để chạy)
npx vitest run       # same
...
- **`shared.mjs`** — Config/env + formatting/body helpers only (read from `.env` via `env()`/`envNum()`): addresses, `RPC_URLS` (server), key-less `PUBLIC_RPC_URLS` (browser), `readBodyLimited()`, `recoverSignerAddress()`. Audit P1.4 split the grab-bag: anti-spam rules → `monitor-rules.mjs`, HMAC auth → `auth.mjs`, cross-process lock → `file-lock.mjs`.
...
- **Tests import the real production modules**; mirror copies were removed (audit A.1b + P1.6):
  ...
  - Remaining debt: `ntfy.test.mjs` builds the ntfy payload locally, mirroring the inline payload in the `monitor.mjs` bootstrap; it also runs live integration tests that POST to `ntfy.sh`.
