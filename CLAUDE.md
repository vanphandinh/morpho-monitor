...
Detects the user's wallet by checking EIP-1193 provider flags: `e.isRabby`, `e.isMetaMask`, `e.isFrame`, `e.isCoinbaseWallet`, `e.isTrust`, `e.isAmbire`. Each detected type gets a tailored UI message. Rabby-specific: `wallet_addEthereumChain` shows a warning about duplicate chainId=1.

Some wallets (Ambire, some MetaMask/Rabby builds) reject custom JSON-RPC methods client-side with `method [morpho_proxyInfo] doesn't has corresponding handler` — the RPC is never reached. When that error string is seen, `checkProxyNetwork()` falls back to `web3_clientVersion`: `MorphoProxy/v1` confirms the proxy (a real node always returns its actual client name, e.g. Geth) and signing is allowed — otherwise wallets like Ambire could never sign even when correctly pointed at the proxy; any other value keeps signing blocked (`unsupported: true`). The "Thêm Mạng Proxy" button gets its own manual-instructions branch for the same rejection of `wallet_addEthereumChain`.
