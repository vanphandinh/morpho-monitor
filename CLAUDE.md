...
# Static gates
npm run lint         # scripts/lint.mjs: oxlint --deny no-undef --deny-warnings .  + TỰ KIỂM độ phủ (bắt lớp lỗi C1)
...
(polling +   (port 3000)      (port 8545)     (VoIP REST  (viem webSocket +
 WSS hybrid)   │                │             API client) closeTransport)
   ...
   │        ├─ webapp-app.mjs            (khởi động + HỢP ĐỒNG window.* — audit P5)
   ...
   │        ├─ webapp-state.mjs          (state cấu hình + ABI + object `state` sealed — audit P5)
   │        ├─ webapp-shell.mjs          (banner DOM, session, UI tài khoản — audit P5)
   │        ├─ webapp-overview.mjs       (tab tổng quan: RPC, market/position — audit P5)
   │        ├─ webapp-presign-bundles.mjs (đọc/xoá bundle đã ký sẵn — audit P5)
   │        ├─ webapp-presign.mjs        (tab ký sẵn: nonce/gas/mốc/ký/lưu — audit P5)
   │        └─ webapp-withdraw.mjs       (tab rút tiền + xác minh tx — audit P5)
   │
...
- **`webapp-handler.mjs`** — Importable pure HTTP request handler (`createRequestHandler({ presignedPath, markets, content, ... })`). All responses are deferred until the registry mutation commits, so no 200 is sent before the lifecycle guard passes. Error codes map to HTTP statuses: `MARKET_INPUT_INVALID` → 400, `MARKET_NOT_CONFIGURED` → 404, `ACTIVE_CLAIM_CONFLICT` → 409, `NONCE_NOT_CLAIMABLE` → 409, `LOCK_STALE` → 503, anything else → 500. POST /api/presign strips client-supplied lifecycle fields (`status`, `txHash`, `rawTx`, `broadcastingAt`, `broadcastingTier`, `minedAt`, `submittedAt`) and persists verified `pending` data only; merging into a broadcasting/submitted/failed bundle is refused, and same-identity records whose status is in `NON_MERGEABLE_STATUSES` (= `HISTORY_STATUSES` + `expired` + `invalid`) are refused with 409 `NONCE_NOT_CLAIMABLE` BEFORE any mutation (diagnosis 2026-09-26, D16). Error bodies now carry the code too (`errorPayload()`: "lỗi nào có `code` thì body có `code`" — audit vòng 2, 2026-09-26): trước đó `code` chỉ map ra HTTP status nên relay proxy KHÔNG có gì để chuyển tiếp và webapp không bao giờ tự phục hồi được (khe giữa hai lớp test đều giả định đầu còn lại đã có `code`). `DELETE /api/presign?market=…&nonce=…` mà KHÔNG kèm `tier` xoá ĐÚNG rung đó (nonce không tồn tại → 400); chỉ khi thiếu hẳn `nonce` mới là xoá cả market. Rung `invalid` là status NON_MERGEABLE duy nhất KHÔNG hàm ý nonce đã tiêu thụ, nên 409 của nó chỉ đúng đường gỡ: xoá rung rồi lưu lại, chữ ký vẫn dùng được. Before that fix an `expired` rung — including an EMPTY one (`withdrawals: []`, exactly the `…@2550` record in the real registry, which also slipped past the merge branch because that branch requires `old.withdrawals.length > 0`) was overwritten as `pending` at the same key, i.e. a dead rung was revived with only the new tier inside: the user's "tier mới tự động được thêm vào bundle nonce cũ". Tests: `__tests__/presigned-expired-rung.test.mjs`. `GET /api/overview` (auth like /api/presign) returns `{ ok, lenderAddress, markets: [{ id, ...registrySummary }] }` for **every** configured market — the multi-market presign dashboard data source (missing bundle → `exists: false`); on-chain nonce is deliberately not included (browser computes duplicate-nonce warnings from bundle nonces).
...
- **`proxy-dispatcher.mjs`** — Importable fake Ethereum JSON-RPC endpoint (`createRpcDispatcher` / `createProxyRequestHandler`). Captures `eth_sendRawTransaction` signed tx hex. **On-chain nonce gate (diagnosis 2026-09-26, D20):** the proxy is the only place that sees BOTH the signed tx and `eth_getTransactionCount(pending)` for the lender, so it refuses a signature whose nonce is already consumed — at capture (`eth_sendRawTransaction`, JSON-RPC error back to the wallet) AND at `POST /bundle` (HTTP 409 `{ ok: false, code: "NONCE_CONSUMED" }`, before the relay, buffer left intact). Rule: block only when on-chain `pending` is strictly GREATER than the tx nonce — a HIGHER nonce is deliberate laddering and stays valid. Missing evidence (RPC down/unsupported) ⇒ fail OPEN with a warning (`readPendingNonce()` logs it): absence of evidence is not evidence the nonce is dead, and D16/D17 still clean up behind. Capture entries record `nonce` + `nonceOnChain`, so `/bundle` reuses capture-time evidence when a hand-built buffer has none; **`GET /captured` EXPOSES those two fields** (still no `signedTx`, nonce is public on-chain) — the only way the browser can learn which nonce a wallet ACTUALLY signed, since the nonce lives solely in the signed bytes. A bundle whose signature nonce differs from the declared one is refused with **409 `{ ok: false, code: "NONCE_MISMATCH", txNonce|txNonces, bundleNonce, index }`** instead of a code-less 400, so the client can tell "the wallet ignored the dApp nonce" from "the bundle is broken" (diagnosis 2026-09-26, "ký ở nonce cao hơn"; `NONCE_MISMATCH` lives in `presign-verify.mjs`, HTTP-level only — the verify codes `VERIFY_*` that the broadcaster keys on are unchanged). Rationale: D19 guards only the browser — a script, a edited wallet nonce, or an old tab could still push a dead-nonce signature, which previously went straight into the buffer/registry. Pure comparator lives in `presign-verify.mjs` (`assertNonceNotConsumed` / `NONCE_CONSUMED`); the webapp surfaces the 409 and immediately re-reads the fresh nonce (`saveToServer`, `result.code === "NONCE_CONSUMED"`). The relay also forwards the SERVER's own error `code` (audit 2026-09-26): a 409 `NONCE_NOT_CLAIMABLE` from `/api/presign` reaches the browser intact, and `saveToServer` recovers from it exactly like the proxy's `NONCE_CONSUMED` (clear reason, fresh nonce, dead signatures invalidated, save button re-enabled only while live signatures remain). Tests: `__tests__/proxy-nonce-freshness.test.mjs` (+ the drill in `__tests__/money-path-e2e.test.mjs`, where pending == tx nonce, so the whole chain still passes). Mocks most methods (chainId, gas, blockNumber) and forwards `eth_getTransactionCount`/`eth_call`/receipts to the real RPC. Handles Rabby-specific methods (`debug_traceCall`, `eth_createAccessList`) with empty responses. `morpho_proxyInfo` → `{ server: "morpho-proxy", chainId: 1 }` is a manual/debug probe only (since 2026-09-24 the webapp no longer calls it — the H5 preflight gate was removed); `web3_clientVersion` stays `"MorphoProxy/v1"`. CORS mirrors the request `Origin` header for credentialed requests. `POST /bundle` matches captured txs with tier metadata via the webapp. `GET /captured` and `DELETE /captured` endpoints for debugging the tx buffer. Mutex-guarded bundle assembly; the capture buffer is capped at `MAX_CAPTURED_TXS` (50, oldest dropped). Since audit vòng 5 (O2) the JSON-RPC branch can be bounded, both off by default: `rpcRateLimit` (per-IP sliding window — over the limit ⇒ JSON-RPC `-32005` with HTTP 200, never 429, because a wallet would treat 429 as a transient network error and retry) and `rpcMethodAllowList` (exported `RPC_METHOD_ALLOW_LIST`, the wallet method set). Wired from `PROXY_RPC_RATE_LIMIT` / `PROXY_ALLOW_PUBLIC_RPC` in `proxy-rpc.mjs`; the limiter only counts the JSON-RPC branch (OPTIONS/`/bundle`/`/captured` unaffected).
...
- **`webapp.html`** — Browser-side SPA served by `webapp-server.mjs`. Uses `viem` `createPublicClient` with a simplified `fallbackTransport()` (round-robin without circuit breaker — browser sessions are short-lived) over `CFG.rpcUrls`, i.e. the server-injected key-less `PUBLIC_RPC_URLS` list (`RPC_URLS` carries API keys and is **never** sent to the browser). A public, **key-less** endpoint list is the only fallback; no provider API key is ever embedded in this file (it is served publicly — see Sharp edges). Detects wallet type (Rabby, MetaMask, Ambire, Frame, Coinbase Wallet, Trust Wallet) via EIP-1193 provider flags. `getCompatibilityMessage()` (now in `webapp-wallet.mjs`) reports wallet brand only (`ok: true` for every wallet — the H5 preflight gate was **removed by user decision on 2026-09-24**; see below). Uses a `_listenersAttached` boolean guard to prevent duplicate event listener registration. Communicates with the webapp server via REST (`/api/challenge`, `/api/auth`, `/api/bundle`, `/api/presign`, `/api/overview`) and with MetaMask via the proxy RPC. The browser code is split into modules `webapp-server.mjs` serves alongside the HTML (audit A.1/A.1b/P2.7/P5): `webapp-app.mjs` (entry: wallet connect, sign-in, tab switching, `init()`, and the **only** place that assigns `window.*`), `webapp-logic.mjs` (pure calc, imported by tests), `webapp-render.mjs` (`esc`/`row`/`formatToken`), `webapp-wallet.mjs` (EIP-1193 brand detection), `webapp-state.mjs` (leaf: config + ABIs + the shared mutable `state` object, `Object.seal`ed so a mistyped field throws instead of silently becoming `undefined`), `webapp-shell.mjs` (banners, proxy URL, session/auth UI), `webapp-overview.mjs` (tab RPC client, market/position fetch + switcher), `webapp-presign-bundles.mjs` (bundle list/read/delete) and `webapp-presign.mjs` (nonce/gas/tiers/sign/save — the only module calling `signTransaction`) and `webapp-withdraw.mjs` (withdraw tab + `txVisibleOnChain` verification). The graph is a DAG (no import cycles): state → shell → overview/bundles → presign → app, and state that only one module uses stays a module-local `let` instead of joining the shared container. `webapp-handler.mjs` serves every configured module through a route map keyed by bare filename (`.mjs` paths → JSON 404 when not wired, never the HTML SPA fallback). Multi-market UX: a market switcher (`#market-switcher`, hidden when only one market is configured) navigates via `?market=<id>`; the presign tab shows an overview block (`/api/overview`) listing every market's bundle status/nonce/tier count and warns when two or more active bundles (pending/broadcasting) share a nonce — only one can ever mine, siblings get expired. **Non-presign "Rút Tiền" tab (audit R4):** `doWithdraw` reports success straight from `walletClient.writeContract`'s hash — and the tx's route is decided by the RPC configured *inside the wallet*. If the wallet points at the proxy (port 8545) the tx is only captured, never broadcast, yet the old UI said "✅ thành công". The banner now appends a **best-effort, non-blocking** verification (`txVisibleOnChain`: 4 tries × 3s against `CFG.rpcUrls`) that warns the user when the hash is still invisible — it never blocks or reverses the success message, since a slow propagation is not evidence of failure (same reasoning that removed the H5 preflight gate). **Presign tab reads SERVER state, not RPC state (diagnosis 2026-09-26, D18/D19):** signed bundles live in the registry, so `init()` calls `fetchExistingBundle()` + `refreshPresignOverview()` from a `finally` (i.e. even when the RPC branch failed) and every `switchTab("presign")` does the same; only the RPC-derived renders (`renderPresignMarketInfo`, `renderPresignPosition`, `updatePresignWalletUI`) stay gated on `state.marketData`. Before the fix a public-RPC failure at page open meant **zero** `/api/presign` requests no matter how often the tab was clicked (`if (tab === "presign" && state.marketData)`), and a transient 503 (`LOCK_STALE`) hid the whole section silently (`if (!resp.ok) return;` + `catch { display = "none" }`) — the reported "thông tin bundle/tier lúc hiện lúc không". Transient failures (503/5xx/fetch throw) now go through `retryTransient()` in `webapp-logic.mjs` (`PRESIGN_FETCH_ATTEMPTS` 3 × `PRESIGN_FETCH_DELAY_MS` 400ms) and a final failure renders a banner with a `Thử lại` button (`window.fetchExistingBundle` / `window.refreshPresignOverview`) instead of hiding. The ladder's `(nonce kế tiếp sẽ broadcast)` marker is attached to the first **live** rung (pending/broadcasting), never to a lower `expired`/`submitted` rung that would read as the current bundle. **Signing re-reads the on-chain pending nonce (D19):** `nonceStillSignable()` runs at the top of `signAllTiers()`/`signWithdrawAll()` — if on-chain is strictly greater than the nonce about to be signed, it blocks, advances `state.presignedNonce` to the fresh floor, invalidates those dead signatures and says why; if the nonce read fails it fails closed (no signature at a blind nonce). A tier signed at a consumed nonce can never mine, and before D16 the POST would have folded it into the old rung. **Signing at a HIGHER nonce is a first-class flow (diagnosis 2026-09-26, follow-up to D19/D20):** the stepper's floor stays the on-chain pending nonce and every nonce ABOVE it stays legal (deliberate laddering), and `#presign-nonce-hint` appears whenever `presignedNonce > onChainPendingNonce` telling the user the wallet must allow a dApp-chosen nonce (MetaMask: Settings → Advanced → “Customize transaction nonce”) — advisory only, no per-wallet preflight gate. After each `sendTransaction` the webapp reads the captured tx through `GET /api/captured` (relayed by `webapp-handler.mjs` the same way `/api/bundle` is: session Bearer → internal Basic) and compares the SIGNATURE's real nonce with `state.presignedNonce`: a mismatch **stops the signing loop immediately** (the remaining confirmations are not burned), marks that tier ❌ with both numbers plus how to fix the wallet, and never lowers `presignedNonce` to the wallet's choice — the rejected alternative was making the webapp "match" the wallet. `saveToServer()` preflights the same evidence before POSTing (no more code-less 400 loop), and handles the proxy's `NONCE_MISMATCH` 409 with a dedicated recovery banner. Missing evidence (`/api/captured` unreachable/old proxy) is NEUTRAL: signing continues and the proxy gate remains the backstop. Tests: `__tests__/webapp-presign-nonce-readback.test.mjs` (+ the frozen webapp trace now carries the `/api/captured` calls and the hint). **Auth mutations refresh the auth-gated sections (diagnosis 2026-09-26, follow-up):** the sign-in button lives INSIDE the presign tab, but `signIn()` used to refresh only the ladder — `#presign-overview` stayed `display:none` until a tab switch or F5 ("Tổng Quan Presign không xuất hiện sau khi xác thực ví"). `signIn()` now also calls `refreshPresignOverview()` (still gated on `currentTab === "presign"`, which keeps the D18 contract that signing in from another tab reads no bundle) and `signOut()` calls both refreshers so every auth-gated section HIDES again; both early-return before any fetch when unauthenticated, so signing out issues no request. `saveToServer` phân nhánh phục hồi bằng chính bằng chứng nonce: `fetchNonce()` trả `boolean`, nonce được đọc TRƯỚC/SAU lần lưu, nên "nonce đã bị tiêu thụ" (ký lại) và "rung cùng nonce chỉ đang chặn, nonce còn sống" (xoá rung rồi Lưu Lại, không ký lại) không còn bị nói lẫn nhau. Nút 🗑 trên mỗi rung không-broadcasting gọi `deleteRungFromBundle()` (xoá ĐÚNG rung, khác ✕ chỉ bỏ một tier và khác nút xoá cả market) — đường gỡ mà banner chỉ phải bấm được thật. Tests: `__tests__/webapp-bundle-visibility.test.mjs`, `__tests__/webapp-stale-nonce-guard.test.mjs`, `__tests__/webapp-gas-decimals.test.mjs`, `__tests__/webapp-auth-overview.test.mjs`, `__tests__/presigned-api.test.mjs`, `__tests__/money-path-e2e.test.mjs`.
...
- **Tests import the real production modules**; mirror copies were removed (audit A.1b + P1.6):
  ...
  - `webapp-app-boot.test.mjs` — imports the real `webapp-app.mjs` with the DOM stub in `helpers/dom-stub.mjs` (built FROM `webapp.html`, so `window`/`location`/`sessionStorage`, `fetch` → 503, `confirm` → false), so the module must actually evaluate and its boot path must run; asserts every `on*` handler in the HTML is a live `window` function (audit vòng 5 O1; tách stub ở vòng 6 P1).
  - `webapp-flows.test.mjs` — chạy THẬT đường tiền (ký sẵn + rút tiền) qua `helpers/webapp-scenario.mjs` rồi khẳng định trên **artifact**: calldata `withdraw()` mà viem gửi cho ví (giải mã lại từng tham số), body `POST /api/bundle`, URL `DELETE /api/presign` (phải kèm nonce), và trạng thái UI sau mỗi bước. Không đọc source. Kịch bản chạy trong tiến trình MỚI (`scripts/webapp-trace.mjs`) vì module ESM được cache theo URL — lượt thứ hai trong cùng tiến trình sẽ không evaluate lại `webapp-state.mjs` và kết quả vô nghĩa (vòng 6, P3).
  - `helpers/webapp-harness.mjs` — RPC giả (encode bằng chính viem), provider EIP-1193 giả, REST giả, DOM stub; `helpers/webapp-scenario.mjs` — kịch bản 19 bước cố định; `__tests__/fixtures/webapp-flows-trace.json` — trace đã ĐÓNG BĂNG (`entry` phải là đường dẫn tương đối; nếu không, fixture ghim đường dẫn của một máy và đỏ trên CI).
  - `lint-gate.test.mjs` — chạy `scripts/lint.mjs` với `--scope` trên thư mục tạm để chứng minh cổng lint TỰ PHÁT HIỆN file ngoài phạm vi quét (lớp lỗi D10) và fail closed khi không parse được số file (vòng 6, P4).
  ...
  - `helpers/browser-modules.mjs` — `browserModuleNames()` suy closure module browser TỪ ĐỒ THỊ IMPORT, và `webappSource()` nối chúng thành "cả webapp". Khẳng định kiểu "webapp có/không có X" dùng nó thay vì ghim vào một file: sau P5, code chuyển nhà giữa 7 module là chuyện thường, còn test ghim file là test đỏ giả (bài học P2.7).
  - Ba bất biến cấu trúc P5 được ghim ở `webapp.test.mjs`: closure == route map của server (thêm module mà quên wire ⇒ đỏ), chỉ `webapp-app.mjs` được gán `window.*`, và mọi `state.<field>` phải là field đã khai báo trong `webapp-state.mjs`.
...
- **Nonce tươi phải được kiểm ở CẢ ba tầng (chẩn đoán 2026-09-26, D19/D20):** browser chặn lúc ký (`nonceStillSignable`, fail closed — không ký thì rẻ), proxy chặn lúc capture và lúc lưu (`assertNonceNotConsumed`, fail open khi không đọc được nonce), server/registry chặn theo trạng thái rung (`409 NONCE_NOT_CLAIMABLE`) + expire/purge. Một tầng bị bỏ ⇒ chữ ký ở nonce đã chết vẫn vào được registry từ phía khác (script tự viết, ví tự thêm nonce, tab cũ).
- **Gas ô nhập + nonce tươi (chẩn đoán 2026-09-26, D15/D18/D19):** hai ô Gwei đi qua `parseGasInput()` (`webapp-presign.mjs`) — KHÔNG `parseFloat` + `String`, vì JS đổi mọi phí dưới `1e-6` Gwei sang ký hiệu khoa học mà `parseUnits` từ chối (`String(parseFloat("0.00000026"))` = `"2.6e-7"`, và `formatUnits(260n, 9)` = `"0.00000026"` chính là giá trị nút Tự Động Gas ghi ra), còn dấu phẩy thập phân kiểu VN bị cắt thành `0` (ô coi như rỗng, im lặng). Quá 9 chữ số thập phân hoặc số mũ ⇒ banner + chặn ký. **`0` là giá trị HỢP LỆ (2026-09-26):** `maxPriorityFeePerGas = 0` (chủ động không tip) ký bình thường; chỉ ô **TRỐNG** mới là "chưa thiết lập". Các guard phải so `=== null` / `!== null && > 0n` chứ KHÔNG dùng falsy — `0n` là falsy trong JS, nên bản cũ khoá nút Ký ngay sau khi nút Tự Động Gas ghi ra `"0"` vì RPC trả `priorityFee = 0`. Trần `maxFeePerGas = 0` thì BỊ CHẶN với thông báo riêng, và cặp phí còn bị chặn khi `tip > trần` (`gasPairError()` — hai luật TĨNH, biên `tip === trần` HỢP LỆ đúng như viem; để lọt thì viem ném `TipAboveFeeCapError` ngay trong vòng ký: tier ❌ với thông báo tiếng Anh, không banner, nút Ký vẫn bật — audit vòng 2, 2026-09-26). `gasPairReady()` là nguồn sự thật chung cho `updateSignButton`/`updatePresignWalletUI`; ô TRỐNG không khoá nút (bấm sẽ nhận lời nhắc "Vui lòng nhập gas"), chỉ cặp VÔ LÝ mới khoá. Test: `__tests__/webapp-gas-decimals.test.mjs`. Ô nhập là `type="text" inputmode="decimal"` (không phải `type="number"`, vốn chặn dấu phẩy trên vài bàn phím). Cùng đợt: bundle đã ký được đọc bất kể RPC sống/chết, và đường ký đọc lại nonce on-chain trước khi ký (xem bullet `webapp.html` ở trên).
...
- Run `npm run lint` before considering a change done: `oxlint --deny no-undef` is the gate that catches the C1-class bug (an identifier removed in one place but still called in another), which `node --check` cannot see. `npm run check` runs lint + `node --check` for every `.mjs` + the full suite. **The lint target is `.` on purpose:** the previous `oxlint … *.mjs __tests__ scripts` relied on the shell expanding `*.mjs`, which npm's Windows shell (cmd.exe) does not do — so on Windows that gate silently linted only the two directories (38 files) and **no root `.mjs` at all** (audit vòng 5, D10: a deliberate `const zzProbe = someUndefinedName123` in `webapp-shell.mjs` left `npm run lint` at exit 0). `oxlint .` lints the same file set deterministically (respecting `.gitignore`) and the probe now fails the gate. **Vòng 6 (P4) khép hẳn lớp lỗi đó:** `npm run lint` giờ là `scripts/lint.mjs`, nó đọc số file mà oxlint **báo đã quét** rồi đối chiếu với danh sách `.mjs` mà git theo dõi (in ra dạng `[lint] ✅ độ phủ: oxlint quét <N> file .mjs (git theo dõi <M> file, phạm vi ".")`; số M tăng theo mỗi file `.mjs` mới được thêm — vòng 6 là 83, vòng 7 là 87 sau khi thêm `__tests__/money-path-e2e.test.mjs`, đợt chẩn đoán 2026-09-26 là 93) — file nằm ngoài phạm vi quét làm cổng ĐỎ, và không parse được con số thì cổng ĐỎ (fail closed), nên "xanh" không còn có thể có nghĩa là "không quét gì". It also checks `webapp.html`: since audit A.1/A.1b/P2.7 there is **no inline module JS left** in that file (only `<script type="importmap">` plus `<script type="module" src="/webapp-app.mjs">`), so the SPA's code is now linted and `node --check`ed like any other `.mjs`, and the old "the SPA can only be verified by opening it in a browser" debt is closed (`node --check` reports it as the `webapp.html no-inline-module` target). `scripts/check-syntax.mjs` still validates the importmap JSON, fails if an inline module ever comes back, and pins that the shared browser modules (`webapp-logic.mjs`, `webapp-render.mjs`, `webapp-wallet.mjs`) exist and are reachable from the import closure of `webapp-app.mjs` (P5: after the split, "app imports it" is no longer the right invariant — "some browser module imports it" is, which still catches a dead module or an import deleted by mistake).
- `npm run lint` là cổng **duy nhất** bắt được lớp C1 trong toàn bộ `.mjs` production; nhưng một cổng chỉ có hiệu lực khi có người gõ nó. Vòng 6 (P5) thêm `.github/workflows/ci.yml`: 4 job `check` (ubuntu + windows × Node 20 + 22) và một job build + smoke image Docker (`scripts/image-module-closure.mjs` chứng minh image sau khi copy `*.mjs` có đủ 10 module browser và đủ file mà importmap trỏ tới). CI cũng là lưới bắt lỗi kiểu "chỉ đúng trên máy tác giả" — vòng 6 đã bắt được đúng một lỗi như vậy: fixture trace ghim đường dẫn tuyệt đối `C:\Users\…` nên suite sẽ đỏ ngay trên runner Linux.
- **`signWithdrawAll()` KHÔNG còn vô hiệu chữ ký đã có (D12, đã sửa):** nó gọi `readGasInputs()` (chỉ ĐỌC) thay vì `onGasInputChange()`, và `onGasInputChange()` chỉ `invalidateSignatures()` khi `gasValuesChanged()` xác nhận gas THỰC SỰ đổi — so **trước khi** gán `presignedGas` (so sau khi gán là so giá trị mới với chính nó ⇒ luôn false ⇒ bảo vệ vô hiệu; lỗi này đã xuất hiện trong quá trình sửa và được ghim bởi ca dương "D12: gas THỰC SỰ đổi thì chữ ký vẫn bị vô hiệu" ở `webapp-flows.test.mjs`). Hành vi CŨ bị phá có chủ ý: fixture trace được đóng băng lại, khác biệt chỉ ở bước `sign-withdraw-all` (không còn banner "Gas đã thay đổi") và `save-to-server-all-shares` (lưu 3 entry thay vì 1); 12 bước trước đó 0 khác biệt so với cây trước P5. Probe đỏ-trước: bỏ guard ⇒ 3 test đỏ; guard luôn-false ⇒ đúng ca dương đỏ.
...
- **ABI `withdraw()` của Morpho nằm ở HAI file, cả hai phải sửa cùng lúc:** `presign-verify.mjs` (`MORPHO_WITHDRAW_ABI` — dùng bởi proxy, webapp API và CLI) và `webapp-state.mjs` (`MORPHO_ABI` — module browser; browser không import được bản server vì nó kéo viem server API). `verify-presigned.mjs` KHÔNG tự định nghĩa ABI (import `MORPHO_WITHDRAW_ABI` từ `presign-verify.mjs`), và `webapp.html` không còn JS inline (A.1/P2.7/P5) nên không giữ ABI nào. *(Đính chính 2026-09-26: câu cũ trong file này nói `verify-presigned.mjs` duplicate và "same ABI is also in webapp.html" — cả hai đều sai và tự mâu thuẫn với mục `verify-presigned.mjs` ở trên.)*
