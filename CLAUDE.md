...
- **`signWithdrawAll()` vô hiệu mọi chữ ký đã có (hành vi hiện tại, đã đo — chưa sửa):** nó gọi `onGasInputChange()` để đọc lại gas, và hàm đó gọi `invalidateSignatures()` **vô điều kiện** — nên bấm "Ký rút toàn bộ shares" reset mọi tier đã ký về `pending` kèm báo "Gas đã thay đổi" **dù gas không đổi**, đồng thời bật lại nút lưu. Kịch bản `helpers/webapp-scenario.mjs` ghim đúng hành vi này (bước `sign-withdraw-all`) nên nếu ai sửa nó thì trace diff sẽ lộ ra ngay. Sửa đúng (chỉ vô hiệu khi gas THỰC SỰ đổi, hoặc gọi `readGasInputs()` không kèm invalidate) là một pha riêng vì nó đụng đường tiền.
